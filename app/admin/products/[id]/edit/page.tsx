...
export default function EditProductPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        const data = await response.json();
        throw new Error(data.error || t('admin.failedToUpdate') || 'Failed to update product');
      }

      router.push('/admin/products');
    } catch (error: any) {
      setError(error.message || t('admin.failedToUpdate') || 'Failed to update product');
      setSaving(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 mb-2">{t('admin.editProduct') || 'Edit Product'}</h2>
          <p className="text-gray-600">{t('admin.editProductDesc') || 'Update product information'}</p>
        </div>
        <Link
          href="/admin/products"
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow-lg border border-gray-200 p-6 space-y-6">
        ...
        <div className="flex gap-4 pt-4">
          ...
          >
            {saving ? (t('common.loading') || 'Saving...') : (t('common.update') || 'Update Product')}
          </button>
          <Link
            href="/admin/products"
