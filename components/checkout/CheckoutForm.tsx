      ...
      >
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
          ...
          <motion.button
            ...
            type="submit"
            disabled={loading}
            className="w-full bg-gradient-to-r from-rose-600 to-rose-500 hover:from-rose-700 hover:to-rose-600 text-white font-semibold py-4 rounded-xl transition-all disabled:opacity-50 shadow-lg shadow-rose-200"
          >
          ...
          {items.map((item, index) => {
            ...
            return (
              <motion.div
                ...
                className="flex justify-between items-center p-4 bg-rose-50 rounded-xl"
              >
                <span className="font-medium text-gray-700">Item x{item.quantity}</span>
                <span className="font-bold text-rose-600">${itemTotal.toFixed(2)}</span>
              </motion.div>
            );
          })}
        </div>
